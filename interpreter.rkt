...
(define m_state_statement
  (lambda (stmt state)
    (cond
      ...
      ((eq? '= (stmt_type stmt)) (m_state_assign (cadr stmt) (caddr stmt) state))
      ((eq? 'var (stmt_type stmt)) (m_state_declare (cadr stmt) state))
      ((eq? 'return (stmt_type stmt)) (toAtoms (state_add 'return (return_helper (m_value (cadr stmt) state)) (state_remove 'return state))))
      ((eq? 'while (stmt_type stmt)) (m_state_while (cond1 stmt) (then-stmt stmt) state)))))      
