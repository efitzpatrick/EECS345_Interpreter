...
(define m_state_statement
  (lambda (stmt state return break continue throw)
    (cond
      ...
      ((eq? 'throw (stmt_type stmt)) (throw (cadr stmt) state))
...
;              (else state)))))  
    (if (m_boolean cond1 state)
        (m_state_while cond1 body
                       (call/cc
                        (lambda (continue) (cdr (m_state_statement (cons block '()) (cdr (m_boolean condition state))
                                                      return
                                                      (lambda (s) (continue (remove_layer state)))
                                                      (lambda (s) (break (remove_layer state)))
                                                      continue))))
                              ...
                              (lambda (leave)
                                (leave (m_state_block
                                        try-body state return break continue (lambda (try-state exception)
                                                                               (leave (m_state_catch catch-stmt exception try-state return break continue throw))))))) return break continue throw)))
...
(define m_state_catch
  (lambda (catch-stmt exception state return break continue throw)
    (if (null? catch-stmt)
        state
        (m_state_block (catch-block catch-stmt) (state_add (throw-val catch-stmt) (m_value exception state) (add_layer state)) return break continue throw))))
