...
(define interpret_parsetree
  (lambda (parsetree state return)
    (if (null? parsetree)
        ...
        (interpret_parsetree (cdr parsetree) (m_state (car parsetree) state return)))))
...
(define interpret
  ...
  (define m_state
    (lambda (expr state return)
      (m_state_statement expr state return)))
  ...
  (define m_state_statement
    (lambda (stmt state return)
      (cond
        ((eq? 'begin (stmt_type stmt)) (m_state_block (stmtlist stmt) state return))
        ((and (eq? 'if (stmt_type stmt))
              (not (eq? (empty_when_no_else stmt) '())))
         (m_state_if_else (cond1 stmt) (then-stmt stmt) (else-stmt stmt) state return))
        ((eq? 'if (stmt_type stmt)) (m_state_if (cond1 stmt) (then-stmt stmt) state return))
        ((and (eq? 'var (stmt_type stmt))
              (not (eq? (empty_when_only_assigning stmt) '())))
         (m_state_declare_assign (declared_var stmt) (assigned_val stmt) state return))
        ((eq? '= (stmt_type stmt)) (m_state_assign (declared_var stmt) (assigned_val stmt) state return))
        ((eq? 'var (stmt_type stmt)) (m_state_declare (declared_var stmt) state return))
        ...
        ((eq? 'while (stmt_type stmt)) (m_state_while (cond1 stmt) (then-stmt stmt) state return)))))
  ...
  (define m_state_block
    (lambda (block state return)
      (if (null? block) 
          state
          (remove_layer (m_state_stmtlist block (add_layer state) return)))))
  ...
  (define m_state_if_else
    (lambda (cond1 then-stmt else-stmt state return)
  ...
  (define m_state_if
    (lambda (cond1 then-stmt state return)
      (if (m_boolean cond1 state)
          (m_state then-stmt state)
          state)))
  ...
  (define m_state_declare_assign
    (lambda (var_name value state return)
